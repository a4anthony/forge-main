...
async function generateDevicesFromModel(viewer, model, dataHelper) {
  ...
  await model.findProperty("SensorName").then((res) => {
    model.getBulkProperties(
      ...
      function (e) {
        let sensors = e.filter((obj) =>
          obj.properties.find(
            (x) => x.displayName === "SensorName" && x.displayValue
          )
        );
        // console.log(sensors)
        sensors.forEach((sensor) => {
          let level = sensor.properties.find(
            (x) => x.displayName === "Schedule Level"
          ).displayValue;
          if (!devices.find((x) => x.id === level)) {
            ...
            sensors
              .filter((obj) =>
                obj.properties.find(
                  (x) =>
                    x.displayName === "Schedule Level" &&
                    x.displayValue === level
                )
              )
              .forEach((sensor) => {
                ...
                sensor.properties
                  .filter(
                    (x) => types.includes(x.displayName) && x.displayValue
                  )
                  .forEach((sensor) => {
                ...
                let positions = dataHelper.getPoints(viewer, model, {
                  id: sensor.properties.find(
                    (x) => x.displayName === "SensorName"
                  ).displayValue,
                  name: sensor.properties.find(
                    (x) => x.displayName === "SensorName"
                  ).displayValue,
                  dbId: sensor.dbId,
                  externalId: sensor.externalId,
                  type: "combo",
                  sensorTypes: sortedTypes,
                });
