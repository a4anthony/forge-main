export default class DataHelper {
    ...
    async getPoints(viewer, model, sensor) {
        // console.log(sensor)
        ...
        let shadingPoint = new SurfaceShadingPoint(
            ...
            sensor.position,
            sensor.sensorTypes,
            sensor.name, { styleId: sensor.styleId || sensor.type }
        );
        if (sensor.dbId != undefined && sensor.position == null) {
            shadingPoint.positionFromDBId(model, sensor.dbId);
            // console.log(shadingPoint.positionFromDBId(model, sensor.dbId))
        }
        return shadingPoint

    }

    async getAllPoints(viewer, model, sensor) {
        let dataVizExt = await this.getExtension(viewer);
        const ns = Autodesk.DataVisualization.Core;
        const {
            SurfaceShadingData,
        ...
        let node = new SurfaceShadingNode(sensor.id, sensor.dbIds);

        sensor.devices.forEach((sensor) => {
            let shadingPoint = new SurfaceShadingPoint(
                sensor.id,
                sensor.position,
                sensor.sensorTypes,
